...
const removeCardFromList = (state, { parentListId, cardIndex }) => ({
  ...
  listCards: {
    ...state.listCards,
    [parentListId]: removeItemFromArray(
      cardIndex,
      state.listCards[parentListId]
    )
  }
});
...
const moveCard = (
  state,
  { parentListId, cardIndex, newParentListId, newCardIndex }
) => ({
  ...state,
  listCards: {
    ...state.listCards,
    ...
    [newParentListId]: addItemToArray(
      state.listCards[parentListId][cardIndex],
      newCardIndex,
      state.listCards[newParentListId]
    )
  }
});

export default function kanbanState(state = {}, action) {
  switch (action.type) {
    case SHOW_CARD_MENUE:
      return setSelectedCard(state, action);
    case SHOW_EDITOR:
      return showEditor(state, action);
    case CLOSE_EDITOR:
    ...
    case CREATE_LIST:
      return addListToListCards(state, action);
    case DELETE_LIST:
      return removeListFromListCards(state, action);
    case SORT_CARD:
      return sortCard(state, action);
    case MOVE_CARD:
